...
class Mappress_Map extends Mappress_Obj {
	...
	*/
	function display($atts = null, $in_iframe = false) {
		...
		if (!$in_iframe && (Mappress::$options->iframes || isset($_REQUEST['mp_compat']))) {
	...
	function display_iframe() {
		...
		// Chrome is VERY fussy about w/h suffixes
		$height = ( (stristr($height, '%') === false) && (stristr($height, 'px') === false) ) ? $height . 'px' : $height;
		$width = ( (stristr($width, '%') === false) && (stristr($width, 'px') === false) ) ? $width . 'px' : $width;

		$wrapper = (stristr($height, '%')) ? "padding-bottom: $height; height: auto" : "padding-bottom: 0; height: {$height}";
		$atts = (array) $this;

		// Only programmatic maps (without a mapid) include POI data in the URL
		// Alternatively: $key = md5($data); set_transient("mapp-iframe-data-$key", $this->pois, 200);
		if ($this->mapid)
			unset($atts['pois']);
